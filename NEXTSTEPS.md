...
## Short Term
- [ ] Migrate oneauth to unified DESIGN.md: delete the 19 doc.go files and 6 diagrams.md files generated by the retired /design-rebuild-go, run /design rebuild --force, refresh MAP.md. Repurpose Makefile `docs-check` (currently scans doc.go for `<!--`) to check every folder with `.design.yaml` also has DESIGN.md.
- [ ] Wire /design check into /start_pr for touched folders (Phase 2)
...
- [ ] Splitting big DESIGN.md (Phase 3 maybe): if any folder's DESIGN.md becomes unwieldy (>2000 lines), document the hand-split convention (`<folder>/DESIGN.md` + `<folder>/designs/<topic>.md`). Don't bake into skill until needed.
